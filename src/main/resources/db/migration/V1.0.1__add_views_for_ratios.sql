...
-- Then ratios can used something like:
-- for university
...
-- SUM(CASE WHEN expected_answer_id = postulant_answer_id THEN 1 ELSE 0 end) / COUNT(pq.id) * 1.0 As ratio_ok
-- FROM postulant_question_with_answers pq
-- JOIN postulant_exam e ON pq.postulant_exam_id = e.id
-- JOIN inscription i on e.inscription_id = i.id 
-- JOIN postulant ON postulant.name = i.postulant_name
-- GROUP BY exam_id, question_number
-- , postulant.upg_code
